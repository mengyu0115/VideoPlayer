    ...
    private fun uploadAndPublishVideo(uri: Uri, title: String, description: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                // Step 2: 构建Multipart请求
                val requestBody = tempFile.asRequestBody("video/*".toMediaTypeOrNull())
                val part = MultipartBody.Part.createFormData("video", tempFile.name, requestBody)

                // Step 3: 上传到服务器
                Log.d(TAG, "uploadAndPublishVideo: 开始上传到服务器")
                val response = withContext(Dispatchers.IO) {
                    RetrofitClient.apiService.uploadVideo(part).execute()
                }
